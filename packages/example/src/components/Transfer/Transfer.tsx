...
export const Transfer: React.FC<ITransferProps> = ({ network, api, onNewMessageCallback }) => {
    const [recipient, setRecipient] = useState<string>("0xdE2819fEc486AC066211CFaC61f7B0E4c8f66006");
    const [amount, setAmount] = useState<string>(Math.random().toFixed(5).toString());
    ...
    const onSubmit = useCallback(async () => {
        if (amount && recipient && api) {
            ...
            // clear form
            setAmount(Math.random().toFixed(5).toString());
            // setRecipient("");
            // setGasFeeCap("0");
            // setGasPremium("0");
            // setGasLimit("0");
